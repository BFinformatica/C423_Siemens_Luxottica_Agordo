...
GestErrore:
    Select Case Err()
        Case -2147467259    'SQL inesistente
            Call WindasLog("CheckDBConnection: Connessione non disponibile!", 1)
            
        Case Else
            Call WindasLog("CheckDBConnection: " & Error(Err()), 1)
    End Select
...
'***** Lettura parametri di configurazione database su file bfdesk.xml *****
    ...
    With AccessObj
        ...
        iTempDB = 0
        Do While Not .IsEOF
            'If .XML_ReadField("DEFAULT") = "Vero" Then
    ...
    defaultIdxConn = DBConnectionsGetDefault()
    
    '** Codice Stazione
    StationCode = connDB(defaultIdxConn).StationCode
    
    Exit Sub
    
GestErrore:
    Call WindasLog("GetConnectionParam " + Error(Err), 1)
...
GestErrore:
    ...
    Call WindasLog("OrderDBConnections: " & Error(Err()), 1)
...
GestErrore:
    ...
    Call WindasLog("DBConnectionsGetDefault: " & Error(Err()), 1)
End Function
...
On Error GoTo Err_CreateRsObject
  ...
  If IsMissing(idxConn) Or idxConn < 0 Then 'se non passo indice di connessione
    idxConn = defaultIdxConn 'prendo quella di default caricata nel getconnectionparam
  End If
