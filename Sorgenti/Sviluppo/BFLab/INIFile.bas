...
Dim nn As Integer
    ...
    For nn = ID_MASTER_1 To ID_MASTER_2
        With rgPLCDef(nn)
            .bInUso = True
            .szDescrizione = "MASTER_" & Trim(Str(nn))
            szIPAddress = "111.111.111.111        "
            'iRetCode = GetPrivateProfileString("MASTER", "IPAddress", "192.168.0.11", szIPAddress, Len(szIPAddress), szFileINI)
            iRetCode = GetPrivateProfileString(.szDescrizione, "IPAddress", "192.168.0.10", szIPAddress, Len(szIPAddress), szFileINI)
            ...
            iRetCode = GetPrivateProfileString(.szDescrizione, "DB", "80", szIPAddress, Len(szIPAddress), szFileINI)
...
GestErrore:
    Debug.Print Now & " ReadFileIni " & Error(Err)
    Resume Next
